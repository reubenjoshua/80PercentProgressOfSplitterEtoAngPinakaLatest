        ...
        try:
            ...
            with open(csv_file_path, 'w', newline='', encoding='utf-8-sig') as csvfile:
                ...
                for atm_ref, transactions in processed_data.items():
                    ...
                    writer.writerow([
                        atm_ref if payment_mode != 'ROB' else next((t.get('atm_reference') for t in transactions if isinstance(t, dict) and t.get('atm_reference')), atm_ref),
                        len(transactions),
    ...
    try:
        ...
        if payment_mode == 'CIS':
            ...
            results = {
                ...
                'total_transactions': sum(data['transaction_count'] for data in grouped_data.values() if isinstance(data, dict))
            }
            ...
            for line in lines:
                ...
                if len(fields) > 4:
                    ...
                    if len(atm_ref_field) >= 4:
                        ...
                        # Store both the original line and the transaction object
                        grouped_data[atm_ref]['raw_contents'].append(line)
                        
                        # Create transaction object with ATM reference
                        transaction = {
                            'payment_mode': payment_mode,
                            'amount': float(fields[6].strip()) if len(fields) > 6 else 0.0,
                            'raw_row': fields,
                            'original_line': line,
                            'atm_reference': atm_ref_field,  # Store the full ATM reference
                            'display_ref': atm_ref,
                            'group_ref': atm_ref
                        }
                        
                        # Store transaction object separately
                        if 'transactions' not in grouped_data[atm_ref]:
                            grouped_data[atm_ref]['transactions'] = []
            ...
            logger.info(f"ROB processing completed")
            return results
